...
// TODO:
class Constants {
    public static get MAX_NUMBER_OF_RESULTS(): number { return 5; }
}
...
class Worker {
    ...
    private generateOutputFiles(dbSource: DatabaseSource) {
        try {
            dbSource.JsFiles.forEach(jsFile => {
                this.generateJsFile(dbSource, jsFile);
    ...
    private generateJsFile(dbSource: DatabaseSource, jsFile: JsFile) {
        //!  var logEntry = Log.Info("Generating output file: {0}", jsFile.Filename);
        let jsNamespace = dbSource.JsNamespace;
        if (!jsNamespace || jsNamespace.trim() == "") jsNamespace = dbSource.MetadataConnection.initialCatalog;

        //!var typeScriptParameterAndResultTypesSB = new StringBuilder();
        let typeScriptParameterAndResultTypesSB: string[] = [];

        let routineContainerTemplate = WorkSpawner.TEMPLATE_RoutineContainer;
        let routineTemplate = WorkSpawner.TEMPLATE_Routine;
        let typescriptDefinitionsContainer = WorkSpawner.TEMPLATE_TypescriptDefinitions;

        //var jsMinifier = new Microsoft.Ajax.Utilities.Minifier();
        ...
        let schemaLookup: { [key: string]: string[]/*Routine defs*/ } = {};
        let tsSchemaLookup: { [key: string]: string[] } = {};


        //     foreach(var r in includedRoutines)
        includedRoutines.forEach(r => {
            let schemaName = r.Schema;
            let routineName = r.Routine;
            let routineType = r.Type.toUpperCase();
            ...
            let jsFunctionName = Worker.makeNameJsSafe(routineName);
            let jsSchemaName = Worker.makeNameJsSafe(schemaName);

            if (schemaLookup[jsSchemaName] == null) {
                schemaLookup[jsSchemaName] = [];
            }

            if (tsSchemaLookup[jsSchemaName] == null) {
                tsSchemaLookup[jsSchemaName] = [];
            }

            // needs to be true so that DAL.js can identify parameters from the [options] parm passed in
            let includeParams: boolean = true;

            if (["PROCEDURE", "FUNCTION", "TVF"].indexOf(routineType) >= 0) {

                let line = routineTemplate.replace(/<<FUNC_NAME>>/g, jsFunctionName)
                    .replace(/<<SCHEMA>>/g, schemaName)
                    .replace(/<<ROUTINE>>/g, routineName);

                if (routineType == "PROCEDURE") line = line.replace(/<<CLASS>>/, "S");
                else line = line.replace(/<<CLASS>>/, "U");

                let jsParameters: string = null;
                let jsParameterComments: string = null;

                let tsParameterTypeDefName: string = null;
                let typeScriptOutputParameterTypeName: string = null;

                if (r.Parameters != null) {
                    tsParameterTypeDefName = `${Worker.makeNameJsSafe(jsNamespace)}_${schemaName}_${jsFunctionName}Parameters`;

                    let sprocParameters = r.Parameters.filter(p => p.ParameterName && p.ParameterName.trim() != "").map(p => {
                        let parmName = p.ParameterName;
                        if (parmName[0] == '@') parmName.substr(1, 9999);
                        return {
                            Name: parmName,
                            IsResult: p.IsResult,
                            DataType: p.DataType,
                    ...
                    //jsParameterComments = string.Join("", (from p in sprocParameters
                    //                                       select string.Format(fieldCommentTemplate, p.Name, string.Format("{0}, {1}", p.JavascriptDataType, p.ParameterMode))).ToArray());


                    jsParameters = sprocParameters.map(p => `"${p.Name}"`).join(',');

                    let tsParameters = sprocParameters.map(p => `${Worker.startsWithNum(p.Name) ? "$" + p.Name : p.Name}${p.HasDefault ? "?" : ""}: ${p.TypescriptDataType}`);
                    ...
                    let outputParms = r.Parameters.filter(p => p.ParameterName && p.ParameterName.trim() != ""
                        && p.IsResult != "YES"
                        && (p.ParameterMode == "INOUT" || p.ParameterMode == "OUT")
                    ).map(p => {
                        let parmName = p.ParameterName;
                        if (parmName[0] == '@') parmName.substr(1, 9999);
                        return `${parmName}?: ${RoutineParameter.getDataTypeForTypeScript(p.DataType)}`;
                    });


                    if (outputParms.length > 0) {
                        typeScriptOutputParameterTypeName = `${Worker.makeNameJsSafe(jsNamespace)}_${schemaName}_${jsFunctionName}OutputParms`;
                ...
                if (jsParameters && jsParameters.trim() != "") {
                    line = line.replace(/<<PARM_DEFINITION>>/g, jsParameters);
                }
                else {
                    line = line.replace(/<<PARM_DEFINITION>>/g, "");
                }
                ...
                if (r.ResultSetMetadata != null) {
                    ...
                    r.ResultSetMetadata.forEach((kv: any) => {
                        let columnCounter: { [key: string]: number } = {};// = new SortedList<string, int>();

                        let tsResultTypeDefName = `${Worker.makeNameJsSafe(jsNamespace)}_${schemaName}_${jsFunctionName}Result${resultIx++}`;
                        let lst: string[] = [];
                        let nonNameColIx: number = 0;
                        ...
                        kv.forEach(col => {
                            ...
                            var dbDataType = col.DataType.name;

                            if (!dbDataType) {
                                let nnn = "something wrong";
                            }

                            if (!colName || colName.trim() == "") {
                                colName = `Unknown${++nonNameColIx}`;
                            }

                            let originalColName = colName;

                            // handle duplicate column names
                            if (columnCounter[colName]) {
                                //!this.Log.Warning("Duplicate column name encountered for column '{0}' on routine {1}", colName, r.FullName);

                                let n = columnCounter[colName];
                                colName += ++n; // increase by one to give a new unique name
                                columnCounter[originalColName] = n;
                            }
                            else {
                                columnCounter[colName] = 1;
                            }

                            colName = Worker.quoteColumnNameIfNecessary(colName);

                            let typeScriptDataType = RoutineParameter.getDataTypeForTypeScript(dbDataType);
                ...
                if (routineType == "FUNCTION") {
                    if (r.Parameters != null) {
                        let resultParm = r.Parameters.find(p => p.IsResult == "YES");
                        let tsMethodStub = `\t\t\tstatic ${jsFunctionName}(parameters?: ${tsArguments}): IUDFExecGeneric<${RoutineParameter.getDataTypeForTypeScript(resultParm.DataType)}, ${tsArguments}>`;
                    ...
                    let outputParmType = "void";

                    // check if there is output parameters present
                    if (typeScriptOutputParameterTypeName && typeScriptOutputParameterTypeName.trim() != null) {
                        outputParmType = "_." + typeScriptOutputParameterTypeName;
                    }

                    if (resultTypes.length > 0) {
                        var cnt = resultTypes.length;
                        ...
                        let resultTypesTopN = resultTypes.slice(0, Constants.MAX_NUMBER_OF_RESULTS).map(rt => `_.${rt}`).join(',');
                ...
                schemaLookup[jsSchemaName].push(line);
            }
            else {
                throw new Error(`Unsupported routine type: ${routineType == null ? "(null)" : routineType}`);
            }

        }); // foreach (var r in routineList)

        let schemaAndRoutineDefs = <any>Object.keys(schemaLookup).map(key => `\tx.${key} = {\r\n\t\t${schemaLookup[key].join(',\r\n\t\t')}\r\n\t}\r\n`).join('\r\n');
        let tsSchemaAndRoutineDefs = Object.keys(tsSchemaLookup).map(key => `\t\tclass ${key} {\r\n ${tsSchemaLookup[key].join(';\r\n')}\r\n\t\t}\r\n`).join('\r\n');

        let finalSB = routineContainerTemplate;// new StringBuilder(routineContainerTemplate);

        jsFile.incrementVersion();

        finalSB = finalSB.replace(/<<DATE>>/g, new Date().toString())
            .replace(/<<FILE_VERSION>>/g, jsFile.Version.toString())
            .replace(/<<Catalog>>/g, Worker.makeNameJsSafe(jsNamespace))
            .replace(/<<ROUTINES>>/g, schemaAndRoutineDefs)
            .replace(/<<DB_SOURCE_GUID>>/g, dbSource.CacheKey.toString());
        ;

        let finalTypeScriptSB = typescriptDefinitionsContainer;


        let resultAndParameterTypes = typeScriptParameterAndResultTypesSB.join('\r\n');


        finalTypeScriptSB = finalTypeScriptSB.replace(/<<DATE>>/, new Date().toString())
            .replace(/<<FILE_VERSION>>/, jsFile.Version.toString())
            .replace(/<<Catalog>>/, Worker.makeNameJsSafe(jsNamespace))
            .replace(/<<ResultAndParameterTypes>>/, resultAndParameterTypes)
            .replace(/<<MethodsStubs>>/, tsSchemaAndRoutineDefs)
            ;

        let typescriptDefinitionsOutput = finalTypeScriptSB;

        let finalOutput = finalSB.toString();

        let outputDir = dbSource.outputDir;
        let filePath = dbSource.outputFilePath(jsFile);
        let minfiedFilePath = dbSource.minifiedOutputFilePath(jsFile);
        let tsTypingsFilePath = dbSource.outputTypeScriptTypingsFilePath(jsFile);

        //!let minifiedSource = jsMinifier.MinifyJavaScript(finalOutput, new Microsoft.Ajax.Utilities.CodeSettings() {  });

        if (fs.existsSync(filePath)) {
            //!var fa = File.GetAttributes(filePath);

            //!if (fa.HasFlag(FileAttributes.ReadOnly)) {
            //!Log.Info("Removing ReadOnly flag on file \"{0}\"", filePath);
            //!File.SetAttributes(filePath, fa & ~FileAttributes.ReadOnly);
            //!}
        }

        if (fs.existsSync(minfiedFilePath)) {
            //!var fa = File.GetAttributes(minfiedFilePath);

            //!if (fa.HasFlag(FileAttributes.ReadOnly)) {
            //!Log.Info("Removing ReadOnly flag on file \"{0}\"", minfiedFilePath);
            //!File.SetAttributes(minfiedFilePath, fa & ~FileAttributes.ReadOnly);
            //!}
        }
        ...
        fs.writeFileSync(filePath, finalOutput, { encoding: "utf8" });
        //!fs.writeFileSync(minfiedFilePath, minifiedSource, { encoding: "utf8" });
        fs.writeFileSync(tsTypingsFilePath, typescriptDefinitionsOutput, { encoding: "utf8" });
    }

    // logEntry.Append(""); // add final duration
}
