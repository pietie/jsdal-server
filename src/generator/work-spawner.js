...
// TODO:
class Constants {
    static get MAX_NUMBER_OF_RESULTS() { return 5; }
}
class WorkSpawner {
    static Start() {
        try {
...
class Worker {
    ...
    generateJsFile(dbSource, jsFile) {
        ...
        includedRoutines.forEach(r => {
            ...
            if (["PROCEDURE", "FUNCTION", "TVF"].indexOf(routineType) >= 0) {
                ...
                if (routineType == "PROCEDURE")
                    line = line.replace(/<<CLASS>>/, "S");
                else
                    line = line.replace(/<<CLASS>>/, "U");
                let jsParameters = null;
                let jsParameterComments = null;
                let tsParameterTypeDefName = null;
                let typeScriptOutputParameterTypeName = null;
                if (r.Parameters != null) {
                    ...
                    let outputParms = r.Parameters.filter(p => p.ParameterName && p.ParameterName.trim() != ""
                        && p.IsResult != "YES"
                        && (p.ParameterMode == "INOUT" || p.ParameterMode == "OUT")).map(p => {
                        let parmName = p.ParameterName;
                        if (parmName[0] == '@')
                            parmName.substr(1, 9999);
                        return `${parmName}?: ${routine_parameter_1.RoutineParameter.getDataTypeForTypeScript(p.DataType)}`;
                    });
                ...
                if (r.ResultSetMetadata != null) {
                    let resultIx = 0;
                    r.ResultSetMetadata.forEach((kv) => {
                        let columnCounter = {}; // = new SortedList<string, int>();
                ...
                if (routineType == "FUNCTION") {
                    if (r.Parameters != null) {
                        ...
                        let tsMethodStub = `\t\t\tstatic ${jsFunctionName}(parameters?: ${tsArguments}): IUDFExecGeneric<${routine_parameter_1.RoutineParameter.getDataTypeForTypeScript(resultParm.DataType)}, ${tsArguments}>`;
                        tsSchemaLookup[jsSchemaName].push(tsMethodStub);
                    }
                    else {
                        log_1.SessionLog.warning(`Cannot generate UDF method stub because the parameters collection is empty. Does the object still exist? Data source= ${dbSource.Name} (${dbSource.CacheKey}), Routine=${r.FullName}`);
                    }
                }
                else {
        ...
        let schemaAndRoutineDefs = Object.keys(schemaLookup).map(key => `\tx.${key} = {\r\n\t\t${schemaLookup[key].join(',\r\n\t\t')}\r\n\t}\r\n`).join('\r\n');
