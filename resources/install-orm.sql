...
if (not exists (select top 1 1 from sys.procedures where name ='SprocGenUpdateResultsetXml' and SCHEMA_NAME(schema_Id) = 'orm'))
BEGIN
	EXEC('
	CREATE PROCEDURE [orm].[SprocGenUpdateResultsetXml]
	(
		@routineId			int
		,@resultsetXml		varchar(max)
		,@rowver			bigint
		,@userErrorMsg		varchar(max) out
	)
	AS
	begin
		DECLARE @outputTable TABLE (Rowver bigint)

		update orm.[SprocDalMonitor] 
			set ResultsetXml = @resultsetXml
			output inserted.rowver into @outputTable
		where Id = @routineId
	
		if (@@ERROR != 0) return;
	
		select @rowver = Rowver from @outputTable
	
		update orm.[SprocDalMonitorAdditional]
			set ResultsetGeneratedFromRowver = @rowver
		where [SprocDalMonitorId] = @routineId
	
		if (@@ROWCOUNT = 0)
		begin
			insert into [SprocDalMonitorAdditional] ([SprocDalMonitorId], [ResultsetGeneratedFromRowver])
			values (@routineId, @rowver)	
		end
...
BEGIN
	...
	begin
		...
		select mon.Id
				...
				,mon.ParameterCount	
				,mon.ResultSetRowver
				,mon.RoutineParsingRowver
				,mon.RoutineParsedResult
...
begin
	...
	begin
		...
		if (exists (select top 1 1 from sys.procedures where name =''SprocGenUpdateResultsetXml'' and SCHEMA_NAME(schema_Id) = ''orm'')) drop proc orm.SprocGenUpdateResultsetXml
